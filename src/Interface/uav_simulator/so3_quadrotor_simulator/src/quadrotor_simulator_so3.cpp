...
main(int argc, char** argv)
{
  ...
  // Initial heading (yaw, rad, CCW about +z; 0 = facing +x, -pi/2 = facing
  // -y). The planner's commanded yaw takes over once intent arrives; this
  // only sets the pre-command hover attitude / RViz appearance.
  if (std::abs(_init_yaw) > 1e-9) {
    QuadrotorSimulator::Quadrotor::State st = quad.getState();
    st.R = Eigen::AngleAxisd(_init_yaw, Eigen::Vector3d::UnitZ())
               .toRotationMatrix();
    quad.setState(st);
  }
  ...
  // Frame ids without leading slashes: tf2 (and thus RViz) rejects frame
  // names starting with '/'. The planner consumes the odom pose directly and
  // does not use TF, so the standard flow is unaffected.
