...
SO3ControlNodelet::onInit(void)
{
  ...
  // Initial yaw reference [rad]: the attitude controller holds this yaw
  // until the first PositionCommand (whose yaw then takes over). Keeps the
  // sim's init_state_yaw from being rotated back to zero before command
  // flow begins.
  n.param("init_yaw", des_yaw_, 0.0);
