...
void SDFMap::updateESDF3d() {
  ...
  // Scanlines are independent (fillESDF scratch is stack-local).
...
void SDFMap::updateESDFCallback(const ros::TimerEvent& /*event*/) {
  ...
  // Always-on 1 Hz stat line for the benchmark harness
  // (scripts/plot_benchmark_metrics.py). Counters are cumulative, so the
  // last occurrence in the log carries the run totals.
  ROS_INFO_THROTTLE(1.0,
      "[SDF][ESDF] update %.2f ms | updates %lu | skipped %lu | max %.2f ms",
      1e3 * (t2 - t1).toSec(),
      static_cast<unsigned long>(esdf_update_count_),
      static_cast<unsigned long>(esdf_skip_count_),
      1e3 * md_.max_esdf_time_);
...
void SDFMap::cloudCallback(const sensor_msgs::PointCloud2ConstPtr& img) {
  ...
  // Static-input fast path: same point multiset + camera still in the same
  // half-resolution cell mean the insertion below and the next updateESDF3d()
  // reproduce the buffers bit-for-bit (occupancy only accumulates here, no
  // reset), so skip both. The per-point hashes are combined with '+' so the
  // hash is ORDER-INSENSITIVE: the upstream kd-tree crop reorders points on
  // sub-mm hover jitter, which a raw byte hash would mistake for new data.
  {
    auto splitmix = [](uint64_t v) {
      v += 0x9E3779B97F4A7C15ULL;
      v = (v ^ (v >> 30)) * 0xBF58476D1CE4E5B9ULL;
      v = (v ^ (v >> 27)) * 0x94D049BB133111EBULL;
      return v ^ (v >> 31);
    };
    const double q = 0.5 * mp_.resolution_;
    uint64_t h = splitmix(static_cast<uint64_t>(static_cast<int64_t>(std::floor(md_.camera_pos_(0) / q))));
    h ^= splitmix(static_cast<uint64_t>(static_cast<int64_t>(std::floor(md_.camera_pos_(1) / q))) + 0x1234567);
    h ^= splitmix(static_cast<uint64_t>(static_cast<int64_t>(std::floor(md_.camera_pos_(2) / q))) + 0x89ABCDE);
    uint64_t acc = 0;
    for (const auto& p : latest_cloud.points) {
      ...
      std::memcpy(&by, &p.y, 4);
      std::memcpy(&bz, &p.z, 4);
      uint64_t v = (static_cast<uint64_t>(bx) << 32) | by;
      v ^= static_cast<uint64_t>(bz) << 16;
      acc += splitmix(v);
    }
    h ^= acc;
    if (h == last_cloud_input_hash_) {
      ++esdf_skip_count_;
      return;
    }
    last_cloud_input_hash_ = h;
  }
