...
private:
  ...
  // FNV-1a of the last fused cloud message (raw bytes) + the quantized camera
  // position. When a new message hashes identically, the occupancy insertion
  // and the ESDF recompute would be bit-identical, so cloudCallback returns
  // early -- static scene + static robot stops paying the 20-40 ms
  // updateESDF3d() every tick.
  uint64_t last_cloud_input_hash_ = 0;
  // Cumulative run counters surfaced in the 1 Hz "[SDF][ESDF]" stat line that
  // scripts/plot_benchmark_metrics.py parses into metrics.json.
  uint64_t esdf_update_count_ = 0;
  uint64_t esdf_skip_count_ = 0;
